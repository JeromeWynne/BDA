...
```{r}
mu_samples <- sample_mu(5, obs_y, var_samples)
{hist(mu_samples, breaks = 100, probability = TRUE,
       ...
       xlim = c(45, 55))
lines(seq(20, 80, 0.05), dt(x = (seq(20, 80, 0.05)-mean(obs_y))/sqrt(sample_var/n_obs), df = n_obs - 1), type = 'l')
lines(seq(20, 80, 0.05), dnorm(x = seq(20, 80, 0.05), mean = mean(obs_y), sd = sqrt(sample_var/n_obs)), type = 'l', col = 'red')}
##  THIS IS WRONG ^ SOMETHING IS GOING AWRY WHEN SAMPLING FROM THE CONDITIONAL POSTERIOR
```
